...
class GameController {
    constructor() {
        ...
        this.selectedDifficulty = null; // 当前选择的难度
    ...
    async init() {
        try {
            ...
            // 初始化难度管理器
            this.difficultyManager = new window.DifficultyManager(this.emperorDatabase);
     ...
     * 随机选择一个皇帝并显示第一个提示词
     * 需求: 1.1
     * @param {'easy'|'medium'|'hard'} difficulty - 选择的难度等级
     * @returns {Object} 新轮次信息
     */
    startNewRound(difficulty = null) {
        if (!this.isInitialized) {
            throw new Error('游戏控制器未初始化');
        }
        
        try {
            console.log(`开始新轮次... 难度: ${difficulty || '未指定'}`);
    ...
    startNextEmperor() {
        ...
        // 随机选择一个未使用的皇帝（根据难度筛选）
        this.currentEmperor = this.emperorDatabase.getRandomEmperor(
            currentState.usedEmperorIds,
    ...
    resetGame() {
        ...
        try {
            ...
            this.currentHints = [];
            this.currentHintIndex = 0;
            this.selectedDifficulty = null; // 重置难度选择
