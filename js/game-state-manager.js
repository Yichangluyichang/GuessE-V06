...
class GameStateManager {
    ...
    validateGameState(state) {
        ...
        // selectedDifficulty 是可选属性（为了向后兼容）
        // 如果不存在，会在加载时自动设置为 null
        if (!('selectedDifficulty' in state)) {
        ...
        if (state.selectedDifficulty !== null) {
            ...
            if (!validDifficulties.includes(state.selectedDifficulty)) {
                console.warn(`无效的难度等级: ${state.selectedDifficulty}`);
                return false;
            }
        }
