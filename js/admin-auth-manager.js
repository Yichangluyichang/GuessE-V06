...
class AdminAuthManager {
    ...
    async setAuthenticated() {
        ...
        if (window.FirebaseSyncManager) {
            try {
                // 使用管理员邮箱和密码登录
                // 注意：这里需要你的 Firebase 管理员邮箱
                const email = 'your-admin-email@example.com'; // 替换为你的管理员邮箱
                const password = this.adminPassword;
                
                const result = await window.FirebaseSyncManager.adminLogin(email, password);
                if (result.success) {
                    console.log('Firebase 管理员登录成功');
                } else {
                    console.warn('Firebase 管理员登录失败:', result.error);
                }
            } catch (error) {
                console.error('Firebase 登录过程中发生错误:', error);
            }
        }
    }
